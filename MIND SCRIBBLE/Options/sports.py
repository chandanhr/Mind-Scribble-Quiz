...
SPORTS_WORD = ['OTCRRA', 'OIRCCOBL', 'ROUFCWIALEL', 'RNOC', 'UCMUECRB', 'GGNALETP', 'EEGRN EREPPP', 'ECETTUL',
                  'OSMROSMUH', 'INNOO', 'OATPTO', 'UNPMIPK', 'RED EPEPPR', 'MOTTOA', 'ETEBTROO', 'EPAS', 'HIRSAD',
                  'CEBABAG', 'CLIHI', 'ICRGAL', 'WETSE OTPAOT', 'RERAOCDIN', ]
SPORTS_ANSWER = ['CARROT', 'BROCCOLI ', 'CAULIFLOWER ', 'CORN ', 'CUCUMBER ', 'EGGPLANT', 'GREEN PEPPER ',
                    'LETTUCE ', 'MUSHROOMS', 'ONION', 'POTATO', 'PUMPKIN ', 'RED PEPPER', 'TOMATO ', 'BEETROOT', 'PEAS',
                    'RADISH', 'CABBAGE', 'CHILI', 'GARLIC', 'SWEET POTATO', 'CORIANDER', ]

ran_num = randrange(0, (len(SPORTS_WORD)))
jumbled_rand_word = SPORTS_WORD[ran_num]
...
def main():
    ...
    def change():
        global ran_num
        ran_num = randrange(0, (len(SPORTS_WORD)))
        word.configure(text=SPORTS_WORD[ran_num])
    ...
    def cheak():
        ...
        if user_word == SPORTS_ANSWER[ran_num]:
            points += 5
    ...
    def show_answer():
        ...
        if points > 4:
            ...
            score.configure(text="Score:- " + str(points))
            time.sleep(0.5)
            ans_lab.configure(text=SPORTS_ANSWER[ran_num])
        else:
    ...
    my_window.title("Quizee-SPORTSs_jumbled_words")
