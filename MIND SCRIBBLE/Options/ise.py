...
ISE_WORD = ['HEDA', 'IHAR', 'EYE', 'SEAR', 'SEON', 'UHMTO', 'NICH', 'OEEAFHDR', 'AJW', 'EKEHC', 'BOEWEYR',
                   'DHRSULOE', 'RSAM', 'ANDH', 'OBWEL', 'NGEIFR', 'GLE', 'FOTO', 'IHGTH', 'NEKE', ]

ISE_ANSWER = ['HEAD', 'HAIR', 'EYE', 'EARS', 'NOSE', 'MOUTH', 'CHIN', 'FOREHEAD', 'JAW', 'CHEEK', 'EYEBROW',
                     'SHOULDER', 'ARMS', 'HAND', 'ELBOW', 'FINGER', 'LEG', 'FOOT', 'THIGH', 'KNEE', ]

ran_num = randrange(0, (len(ISE_WORD)))
jumbled_rand_word = ISE_WORD[ran_num]
...
def main():
    ...
    def change():
        global ran_num
        ran_num = randrange(0, (len(ISE_WORD)))
        word.configure(text=ISE_WORD[ran_num])
    ...
    def cheak():
        ...
        if user_word == ISE_ANSWER[ran_num]:
            points += 5
    ...
    def show_answer():
        ...
        if points > 4:
            ...
            score.configure(text="Score:- " + str(points))
            time.sleep(0.5)
            ans_lab.configure(text=ISE_ANSWER[ran_num])
        else:
    ...
    my_window.title("Quizee-ISE_jumbled_words")
