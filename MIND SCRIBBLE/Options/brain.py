...
BRAIN_WORD = ['EALPP', 'VAAGU', 'PEHCA', 'PRAE', 'NGMAO', 'PAAPYA', 'EGNRAO', 'AGESPR', 'WIKI', 'RRYCHE', 'LRTOANWEME',
               'PEELIPNPA', 'BELUEBRRY', 'NABAAN', 'NTUOCCO', 'SCURATD EALPP', 'MONLE', 'EREYBMUL', 'MARTAIND']

BRAIN_ANSWER = ['APPLE', 'GUAVA', 'PEACH', 'PEAR', 'MANGO', 'PAPAYA', 'ORANGE', 'GRAPES', 'KIWI', 'CHERRY',
                 'WATERMELON', 'PINEAPPLE', 'BLUEBERRY', 'BANANA', 'COCONUT', 'CUSTARD APPLE', 'LEMON', 'MULBERRY',
                 'TAMARIND']

ran_num = randrange(0, (len(BRAIN_WORD)))
jumbled_rand_word = BRAIN_WORD[ran_num]
...
def main():
    ...
    def change():
        global ran_num
        ran_num = randrange(0, (len(BRAIN_WORD)))
        word.configure(text=BRAIN_WORD[ran_num])
    ...
    def cheak():
        ...
        if user_word == BRAIN_ANSWER[ran_num]:
            points += 5
    ...
    def show_answer():
        ...
        if points > 4:
            ...
            score.configure(text="Score:- " + str(points))
            time.sleep(0.5)
            ans_lab.configure(text=BRAIN_ANSWER[ran_num])
        else:
